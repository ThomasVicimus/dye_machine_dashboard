...
def stacked_rfc_clf(
    ...
    try:
        ...
        print(">>>>>Classification : Stacked RFC Classification Starts.\n")
        ...
        # * xgb param
        clf_config["xgb_param"]["num_class"] = len(y)
        xgb_clf = xgb.XGBClassifier(**clf_config["xgb_param"])
        xgb_clf.fit(transformed_features, y)
        xgb_proba = xgb_clf.predict_proba(transformed_features)

        # * rfc param
        rfc_clf = RandomForestClassifier(**clf_config["rfc_param"])
        rfc_clf.fit(transformed_features, y)
        rfc_proba = rfc_clf.predict_proba(transformed_features)

        stack_proba = np.hstack([rfc_proba, xgb_proba])
        X = stack_proba
        ...
        for store in output_paths.keys():
            ...
            if len(df_cleansed) > 0:
                ...
                xgb_proba = xgb_clf.predict_proba(test_transformed_features)
                rfc_proba = rfc_clf.predict_proba(test_transformed_features)
                test_stack_proba = np.hstack([rfc_proba, xgb_proba])

                y_hat = y_encoder.inverse_transform(clf.predict(test_stack_proba))
